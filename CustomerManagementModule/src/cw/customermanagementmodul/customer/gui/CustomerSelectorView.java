...
import cw.customermanagementmodul.customer.gui.CustomerSelectorPresentationModel.CustomerSelectorFilterExtentionPointHelper;
...
public class CustomerSelectorView
	extends CWView<CustomerSelectorPresentationModel>
{
    ...
    public void initComponents() {
        ...
        bFilterSwitcher = CWComponentFactory.createButton(getModel().getFilterSwitcherAction());
        bFilterSwitcher.setHorizontalAlignment(SwingConstants.LEFT);
        bFilterSwitcher.setUI(new CWButtonPanelButtonUI());
        bFilterChooser = CWComponentFactory.createButton(getModel().getFilterChooserAction());
        ...
        tCustomers = CWComponentFactory.createTable(
        		getModel().getCustomerDataModel(),
                "Keine Kunden vorhanden",
                getModel().getCustomerTableStateName()
                );
        tCustomers.setSelectionModel(
                new SingleListSelectionAdapter(
                    new CWTableSelectionConverter(
                    	getModel().getCustomerDataModel().getSelectionIndexHolder(),
                        tCustomers)));
...
//        tCustomers.getColumns(true).get(16).setCellRenderer(new DateTimeTableCellRenderer(CalendarUtil.DATETIME_FORMAT_STANDARD));
        ...
        List<CustomerSelectorFilterExtentionPointHelper> filterExtentionsHelper = getModel().getFilterExtentionsHelper();
    ...
    private void initEventHandling() {
        ...
        getModel().getFilterSwitcherModel().addValueChangeListener(filterSwitcherChangeListener = new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                // If the filter gets opened and there are filters active, open it
                if((Boolean)evt.getNewValue() == true && (Integer)getModel().getActiveFilterCountModel().getValue() > 0) {
                    pFilter.setVisible((Boolean) getModel().getFilterSwitcherModel().getValue());
                } else {
                    // If the filter gets opened and there are no filters active, show the popup
                    if((Boolean)evt.getNewValue() == true && (Integer)getModel().getActiveFilterCountModel().getValue() == 0) {
                        showFilterPopup();
                        getModel().getFilterSwitcherModel().setValue(false);
                    } else {
                        pFilter.setVisible((Boolean) getModel().getFilterSwitcherModel().getValue());
                    }
                }
        ...
        getModel().getActiveFilterCountModel().addValueChangeListener(filterSwitcherPanelCountChangeListener = new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                // Hide the panel if the last filter is removed
                if((Integer)getModel().getActiveFilterCountModel().getValue() == 0 && (Boolean)getModel().getFilterSwitcherModel().getValue() == true) {
                	getModel().getFilterSwitcherModel().setValue(false);
                }
                // If the first filter is added and the panel is not shown, show it
                if((Boolean)getModel().getFilterSwitcherModel().getValue() == false && (Integer) evt.getOldValue() == 0 && (Integer) evt.getNewValue() == 1) {
                	getModel().getFilterSwitcherModel().setValue(true);
//                    pFilter.setVisible(true);
        ...
        getModel().getFilterSwitcherModel().addValueChangeListener(filterSwitcherIconChangeListener = new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                if((Boolean) getModel().getFilterSwitcherModel().getValue() == true){
        ...
        getModel().getFilterEnabledModel().addValueChangeListener(filterEnabledChangeListener = new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                pFilterHeader.setVisible((Boolean)getModel().getFilterEnabledModel().getValue());
                if(!pFilterHeader.isVisible()) {
                    pFilter.setVisible((Boolean)getModel().getFilterEnabledModel().getValue());
                }
            }
        });
        pFilterHeader.setVisible((Boolean)getModel().getFilterEnabledModel().getValue());
    ...
    public void buildView() {
        ...
        for(int i=0, l=filterExtentionsHelper.size(); i<l; i++) {
            ...
            CWButton disableButton = CWComponentFactory.createButton(new CWAction("", CWUtils.loadIcon("cw/boardingschoolmanagement/images/filter_remove.png")) {
    ...
    public void dispose() {
        ...
        getModel().getFilterSwitcherModel().removeValueChangeListener(filterSwitcherChangeListener);
        getModel().getFilterSwitcherModel().removeValueChangeListener(filterSwitcherIconChangeListener);
        getModel().getFilterChooserAction().removePropertyChangeListener(filterEnabledChangeListener);
        getModel().getActiveFilterCountModel().removeValueChangeListener(filterSwitcherPanelCountChangeListener);
